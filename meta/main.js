...
function renderScatterPlot(data, commits) { 
    ...
    xScale = d3
        ...
        .domain([d3.timeDay.offset(d3.min(commits, d => d.date), -1),  // one day before
            d3.timeDay.offset(d3.max(commits, d => d.date), +1)]) // one day after
        .range([0, width])
        .nice(d3.timeDay);
...
function brushed(event) {
  ...
  d3.selectAll('circle').classed('selected', (d) =>
    isCommitSelected(selection, d), renderSelectionCount(selection),
  );
}
